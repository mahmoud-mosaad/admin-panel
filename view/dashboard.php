        ...
        <?php if(!empty($userRoles) &&$userRoles[2]->auth == 1) : ?>
            <td>
                <button class='btn btn-primary' name="edit">
                    <a class='simple_button' href="index.php?controller=UserController&method=edit&edit=<?php echo $user->id; ?>">
                        Edit
                    </a>
                </button>
            </td>
        ...
        <?php if(!empty($userRoles) &&$userRoles[3]->auth == 1) : ?>
            <td>
                <button class='btn btn-danger'>
                    <a class='simple_button' href="index.php?controller=UserController&method=delete&delete=<?php echo $user->id; ?>">
                        Delete
                    </a>
                </button>
            </td>
